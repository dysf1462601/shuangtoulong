...
【注意】本任务只需进入目标作者的视频播放页，绝对不要做评论或点赞等多余动作。""",
    ...
    # 🌟 新增：BOSS直聘打招呼模式
    "NAV_BOSS": """【任务：BOSS直聘页面纠偏与翻页检查】
你的任务是确保我们在正确的页面，并为下一步打招呼准备好新岗位。请严格按以下步骤执行，做完即结束：
1. 【全局纠偏】：首先观察当前处于什么页面。
   - 如果你在聊天界面、职位详情页、我的页面或跳到了其他APP -> 请一直执行返回操作(Back)，直到你回到BOSS直聘的主页（特征：底部有“职位”、“发现”、“消息”、“我的”导航栏，且当前显示的是职位列表）。
2. 【视觉判断与翻页】：确认在职位列表页后，仔细观察屏幕上的岗位卡片标题颜色。
   - ⚠️视觉特征提示：你之前点击过或沟通过的岗位，其标题字体会**变灰（暗色）**。未点击过的新岗位标题是**正常的黑色/亮色**。
   - 【情况A】：如果当前屏幕上**所有**可见的岗位标题都已经变灰，说明这页看完了，请**向上滑动屏幕一次**（Swipe）加载新岗位，然后宣告任务完成。
   - 【情况B】：如果当前屏幕上**还有未变灰**（正常黑色字体）的岗位，说明有新岗位，**千万不需要滑动**，直接宣告任务完成退出。
【安全红线】
- 🚫 本任务只负责页面恢复和翻页找新岗位，绝对不要在这个任务中点击任何岗位进入详情页！""",

    "ACTION_BOSS": """【任务：BOSS直聘自动打招呼与闭环返回】
你现在的目标是对一个新岗位打招呼，并安全退回列表。请严格按以下逻辑执行：
1. 【异常检查】：如果你当前不在职位列表页，请一直执行返回操作，直到回到职位列表页。
2. 【寻找目标】：在职位列表页中，寻找一个**未变灰**（正常黑色字体、未被点击过）的岗位卡片，点击进入该职位详情页。
3. 【详情页操作】：进入详情页后，观察底部按钮：
   - 如果底部是【立即沟通】：请点击“立即沟通”。页面会跳转到聊天界面（系统通常会自动发招呼），此时请立刻点击左上角“<”或执行返回操作，**一直退回到职位列表页**。
   - 如果底部是【继续沟通】、【已沟通】或弹出“职位已关闭”：说明此岗位无效或已聊过，请直接点击左上角“<”或执行返回操作，**一直退回到职位列表页**。
4. 【任务结束】：只要你成功退回到了包含多个岗位的“职位列表页”，立即使用 finish 宣告任务完成。
【安全红线】
- 🚫 绝对不要点击任何付费、简历购买、VIP、加微信等无关按钮。
- 🚫 【核心要求】必须确保最后退回到了职位列表页才能宣告完成，绝对不能停留在聊天框里！"""
}
...
def generate_boss_nav_task():
    """BOSS直聘模式：列表页检查与翻页任务"""
    return TASK_TEMPLATES["NAV_BOSS"]

def generate_boss_action_task():
    """BOSS直聘模式：打招呼动作任务"""
    return TASK_TEMPLATES["ACTION_BOSS"]
...
def main():
    ...
    if RUN_MODE == "COLLECT":
    ...
    elif RUN_MODE == "BOSS_GREET":
        # ==================== BOSS直聘打招呼模式 ====================
        print(f"📦 BOSS直聘打招呼模式已启动，将自动寻找岗位并打招呼...")
        print("=" * 50)
        ...
        while True:
            try:
                boss_count += 1
                print("\n" + "=" * 50)
                print(f"📍 【第{boss_count}轮 · 第1步：检查与翻页】")
                print("=" * 50)
                nav_task = generate_boss_nav_task()
                run_main(nav_task)
                ...
                print("\n" + "=" * 50)
                print(f"📍 【第{boss_count}轮 · 第2步：打招呼】")
                print("=" * 50)
                action_task = generate_boss_action_task()
                run_main(action_task)
                ...
                print(f"\n用户中断，程序退出。共完成 {boss_count} 轮打招呼。Goodbye!")
                break
            except Exception as e:
                print(f"\n❌ 处理过程发生错误: {e}")
                time.sleep(5)
